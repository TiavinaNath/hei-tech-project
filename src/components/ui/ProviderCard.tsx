...
export default function ProviderCard({ provider }: { provider: any }) {
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-white rounded-xl shadow-md space-y-4 max-w-sm mx-auto">
      {/* PHOTO DE PROFIL */}
      ...
      <div className="w-full mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        {provider.provider_engagements &&
        provider.provider_engagements.length > 0 ? (
          provider.provider_engagements.map((engagement: any, idx: number) => (
            <div
              key={engagement.id ?? idx}
              className="border border-gray-200 rounded-lg p-4 bg-gray-50 shadow-sm"
            >
              <p className="text-gray-700 font-medium">{engagement.label}</p>
            </div>
          ))
        ) : (
